...
*/
const handleBlockScopedDataMessage = async (response, registry,ws) => {
    ...
    if (output !== undefined) {
        ...
        console.log(JSON.stringify(outputAsJson.mapOutput.data ,null,4))

       if(ws)
         ws.send(JSON.stringify(outputAsJson.mapOutput.data,null,4));
...
export const handleResponseMessage = async (message, registry,ws) => {

     switch(message.case) {
        case "blockScopedData":
            handleBlockScopedDataMessage(message.value, registry,ws);
            break;

        case "blockUndoSignal":
            handleBlockUndoSignalMessage(message.value,ws);
            break;
        case "progress":
